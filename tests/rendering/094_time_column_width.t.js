StartTest(function(t) {
    ...
    t.it('forceFit and columns need to be fitted, weekAndDayLetter', function (t) {
        ...
        t.isApprox(
            headerEl.down('table td').getRight(),
             headerEl.down('.sch-header-row-bottom td:nth-child(7)').getRight(),
    ...
    t.it('No forceFit, but snapToIncrement. Months do not support snapping.', function (t) {
        ...
        t.is(view.getTimeColumnWidth(), presetColWidth, 'monthAndYear #1 Column width matches view preset setting.');
        scheduler.switchViewPreset('monthAndYear', new Date(2010, 6, 1));
        t.is(view.getTimeColumnWidth(), presetColWidth, 'monthAndYear #2 Column width matches view preset setting.');
    })
    ...
    t.it('No forceFit and no snapToIncrement, and no need to expand the columns to fit the available width. Should just use the column width value from viewPreset.', function (t) {
        ...
        t.is(view.getTimeColumnWidth(), presetColWidth, 'Column width matches view preset setting.');
    })
    ...
    t.it("No forceFit and no snapToIncrement, but columns don't consume entire available space and should grow to fit the available width.", function (t) {
        ...
        var view = scheduler.getSchedulingView();
        t.isApprox(view.getTimeColumnWidth() * 4, scheduler.getTimeAxisColumn().getAvailableWidthForSchedule(), 'Correct width when columns do not consume the whole available space');
    })
    ...
    t.it("No forceFit and no snapToIncrement, but columns don't consume entire available space and should grow to fit the available width.", function (t) {
        ...
        var view = scheduler.getSchedulingView();
        t.is(view.getTimeColumnWidth(), Math.floor(scheduler.getTimeAxisColumn().getAvailableWidthForSchedule()/view.timeAxis.getCount()), 'forceFit applied the correct column width');
    })
    ...
    t.it("No forceFit and no snapToIncrement, but columns don't consume entire available space and should grow to fit the available width.", function (t) {
        ...
        t.is(view.getTimeColumnWidth(), (24*60/15), 'Correct column width when snapToIncrement and resolution affects the column width');
    })
    ...
    t.it("No forceFit and no snapToIncrement, but columns don't consume entire available space and should grow to fit the available width.", function (t) {
        ...
        // Since columns don't consume all space, their size will be doubled
        t.is(Math.floor(snapAmount), snapAmount, 'view.getSnapPixelAmount() returns an even number: ' + snapAmount);
        t.is(view.getTimeColumnWidth(), 2*(24*60/15), 'Correct column width when snapToIncrement and resolution affects the column width');
    })
    ...
    t.it("No forceFit and no snapToIncrement, but columns don't consume entire available space and should grow to fit the available width.", function (t) {
        ...
        t.is(view.getTimeColumnWidth(), 139, 'Correct column width when snapToIncrement and resolution affects the column width');
    })
});
