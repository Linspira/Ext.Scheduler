StartTest(function(t) {
    ...
    var iframe              = document.body.appendChild(document.createElement('iframe')),
        ...
        lockedBodySelector = scheduler.lockedGrid.view.getBodySelector(),
        normalBodySelector = scheduler.normalGrid.view.getBodySelector(),
        doc,
        lockedHeader,
        lockedBody,
        normalBody;
    ...
    var checkOutputColumns  = function (t, visible, hidden) {
        ...
        for (var i = 0; i < visible.length; i++ ) {
            columnResult = t.$('.x-column-header:contains("' + visible[i] + '")', lockedHeader);
            t.is(columnResult.length, 1, 'locked column ' +  visible[i] + ' is visible');
        }

        if (hidden) for (i = 0; i < hidden.length; i++ ) {
            columnResult = t.$('.x-column-header:contains("' + hidden[i] + '")', lockedHeader);
            t.notOk(columnResult.length, 'locked column ' +  hidden[i] + ' is hidden');
        }
    };

    var checkOutputColumnCells = function (t, contains, notContains) {
        var columnResult;

        columnResult = t.$('.x-grid-cell:contains("' + contains + '")', lockedBody);
        t.ok(columnResult.length, 'locked column cell contains: ' +  contains + '');

        columnResult = t.$('.x-grid-cell:contains("' + notContains + '")', lockedBody);
        t.notOk(columnResult.length, 'locked column cell does not contain: ' +  notContains + '');
    };
    ...
    var checkPanelColumns   = function (t) {
        ...
        t.ok(columns[7].isVisible(), 'Visible column Id is shown');
    };
    ...
    t.waitForRowsVisible(scheduler, function () {
        ...
        function (response){
            ...
            t.it('0th page has correct set of columns marked as visible', function (t) {
                ...
                checkOutputColumns(t, ['Name0', 'Name2', 'Name4', 'Name5', 'Name6'], ['Id']);
                checkOutputColumnCells(t, 'Mike', 'r1');
            });

            t.it('1th page has correct set of columns marked as visible', function (t) {
                // put 1th page into iframe
                setIframe(pages[1].html);
                checkOutputColumns(t, ['Id'], ['Name0', 'Name2','Name4', 'Name5', 'Name6']);
                checkOutputColumnCells(t, 'r1', 'Mike');
                checkEvents(t);
            });
