...
Class('Bryntum.Test', {
    ...
    methods : {
        ...
        waitForCSSTransition : function (el, cb) {
            if (!this.Ext().isIE9m) {
                var func = function () {
                    el.dom.removeEventListener('transitionend', func);
                    cb();
                }
                el.dom.addEventListener('transitionend', func);
            } else {
                cb();
            }
        },
        ...
        getResourceTreeStore : function (config) {
            ...
            var treeStore = Ext.create('Sch.data.ResourceTreeStore', Ext.apply({
                model : 'Sch.model.Resource',
        ...
        columnLinesSynced   : function (scheduler, description) {
            if (scheduler.columnLines && scheduler.isHorizontal()) {
                ...
                for (var i = 0; i < lines.elements.length; i++) {
                    var headerPos       = Ext.get(headerCells.elements[i]).getBox().right
                    var linePos         = Ext.get(lines.elements[i]).getBox().right
        ...
        getAllPlugins : function() {
            ...
            return Ext.Array.map(Ext.ClassManager.getNamesByExpression('Sch.plugin.*'), function(cls) {
                ...
                if (cls.match(/Lines|Zones/)) {
                    return Ext.create(cls, { store : new Ext.data.Store({ proxy : 'memory' })});
                }
