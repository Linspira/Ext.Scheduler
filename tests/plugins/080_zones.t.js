    ...
    var innerTest = function (t, isTree, orientation) {
        ...
        t.chain(
                ...
                waitFor : function () {
                    ...
                    if (orientation === 'horizontal') {
                        return els.getCount() > 0 &&
                            Math.abs(els.first().getHeight() - viewEl.down('table').getHeight()) < 2;
                    } else {
                        return els.getCount() > 0 &&
                            Math.abs(els.first().getWidth() - viewEl.child('table').getWidth()) < 2;
                    }
                } 
            },
            ...
            function (next) {
                ...
                if (orientation === 'horizontal') {
                    t.isApprox(zoneEl.getHeight(), viewEl.down('table').getHeight(), 1, 'Height correct after resource record was added');
                }
