StartTest(function (t) {
    ...
    t.it('Should select multiple cells on click with ctrl key being hold', function (t) {
        ...
        t.chain(
            { click : ".x-grid-row:nth-child(2) .sch-timetd", offset : [75, 13] },
            function (next) {
                boxes.push(scheduler.el.down('.sch-cellplugin-highlighter.active').getBox());
                next();
            },
            { click : ".x-grid-row:nth-child(3) .sch-timetd", offset : [164, 17], options : { ctrlKey : true } },
            function (next) {
                boxes.push(scheduler.el.down('.sch-cellplugin-highlighter.active').getBox());
                next();
            },
            { click : ".x-grid-row:nth-child(4) .sch-timetd", offset : [279, 17], options : { ctrlKey : true } },
            function (next) {
                boxes.push(scheduler.el.down('.sch-cellplugin-highlighter.active').getBox());
                next();
            },
            { click : ".x-grid-row:nth-child(5) .sch-timetd", offset : [406, 19], options : { ctrlKey : true } },
            function (next) {
    ...
    t.it('Multiple selection should save box sizes', function (t) {
        ...
        t.chain(
            ...
            { click : ".x-grid-row:nth-child(1) .sch-timetd", offset : [174, 9] },
            { click : ".x-grid-row:nth-child(2) .sch-timetd", offset : [154, 19], options : { ctrlKey : true } },
            { click : ".x-grid-row:nth-child(3) .sch-timetd", offset : [291, 18], options : { ctrlKey : true } },
            function (next) {
                var boxes   = scheduler.el.query('.sch-cellplugin-highlighter.clone');
                var v       = scheduler.getSchedulingView();
                
                var boxEl   = Ext.get(boxes[0]);
                t.isApprox(boxEl.getHeight(), Ext.fly(v.getNodeById('r1')).getHeight(), 1, 'Box height is correct');
    ...
    t.it('Should clear selection on simple cell click', function (t) {
        ...
        t.chain(
            ...
            { click : ".x-grid-row:nth-child(4) .sch-timetd", offset : [279, 17], options : { ctrlKey : true } },
            { click : ".x-grid-row:nth-child(5) .sch-timetd", offset : [406, 19], options : { ctrlKey : true } },
            { click : ".x-grid-row:nth-child(6) .sch-timetd", offset : [437, 12] },
            function (next) {
                t.is(scheduler.el.query('.sch-cellplugin-highlighter').length, 1, 'Selection removed');
            }
        );
