StartTest(function (t) {
    ...
    var getCellPosition = function (col, row) {
        ...
        return [viewXY[0] + width * col, view.el.down('.x-grid-table tr:nth-child(' + (row + 1) + ')').getY()];
    };
    ...
    var checkDimensions = function (t, col, row, eId) {
        ...
        box.select('.sch-cellplugin-border-vertical').each(function (el) {
            result = result && Math.abs(el.getHeight() - height) <= 1;
        });
    ...
    t.it('Events should be seleted properly', function (t) {
        ...
        t.chain(
            ...
            { click : 'schedulergrid gridpanel => tr.x-grid-row:nth-child(2) .x-grid-cell' },
            function (next) {
