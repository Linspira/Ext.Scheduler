StartTest(function (t) {
    ...
    var scenario = function (t, config) {
        ...
        // step to wait for CSS3 transition completion (if supported by browser)
        var waitForAnimation    = Ext.isIE9m ? { waitFor : 0 } : { waitForEvent : [ s.el, 'transitionend' ] };
        ...
        t.chain(
            ...
            function (next) {
                // we have to wait for CSS3 transition completion before evaluating element size
                Ext.isIE9m ? t.waitForEvent(s.eventStore, 'add', next) : t.waitForEvent(s.el, 'transitionend', next);
                s.eventStore.add({
                    ResourceId : 'r1',
