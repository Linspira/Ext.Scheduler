StartTest(function(t) {
    ...
    function testRTL() {
        ...
        var timeAxisCol = view.getTimeAxisColumn();

        t.is(view.getDateFromCoordinate(timeAxisCol.el.down('.sch-header-row-middle .sch-column-header:nth-child(1)').getRight(), 'round'),
            new Date(2009, 1, 1),
            'RTL: Should find start date at the left edge of 1st time column');

        view.scrollHorizontallyTo(view.el.dom.scrollLeft-tickWidth);
        t.is(view.getDateFromCoordinate(timeAxisCol.el.down('.sch-header-row-middle .sch-column-header:nth-child(2)').getRight(), 'round'),
            new Date(2009, 1, 2),
