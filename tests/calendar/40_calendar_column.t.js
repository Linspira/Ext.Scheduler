StartTest(function (t) {
    ...
    t.xit('Column resize should update zones', function (t) {
        ...
        t.chain(
            ...
            { drag : '.x-grid .x-grid-header-ct .x-column-header:nth-child(7)', offset : [0, 9], by : [-77, 1] },
            function (next) {
                var cell = scheduler.el.down('.x-grid-body tr:nth-child(1) .sch-timetd:nth-child(6)').getBox();
                var zone = scheduler.el.down('.zone1').getBox();
                
                t.diag('Zone for thin column');
                matchCellAndZone(cell, zone, t);
                
                cell = scheduler.el.down('.x-grid-body tr:nth-child(1) .sch-timetd:last()').getBox();
